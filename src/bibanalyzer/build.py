...
def book2authors(fields):
    ...
    for fld in fields:
        ...
        if len(['1' for c in fld if c in '0123456789:()']) > 0:
            continue
            # комбинации с числами
        # Слова которые не должны считаться фамилиями
        fd = fld.replace(' ', '.').split('.')
        #
        if len([
                '1' for w in wrong_words
    ...
    for a in authors_list:
        ...
        if len(s) == 1:
            continue
            # Фамилия не может быть длиной в одну букву, это ошибка
...
def localize(s):
    book_txt = str(s)
    book_lang = ""
    lat_c = len(['1' for c in book_txt if 'A' <= c <= 'z'])
    # количество латинских символов
    rus_c = len(['1' for c in book_txt if 'А' <= c <= 'я'])
    # количество русских символов
    if lat_c > rus_c:
        book_lang = 'L'
        for i in r2l:
            book_txt = book_txt.replace(i, r2l[i])
    else:
        book_lang = 'R'
        for i in l2r:
            book_txt = book_txt.replace(i, l2r[i])
        for i in l2r_oneway:
            book_txt = book_txt.replace(i, l2r_oneway[i])
        ...
        if current_dis['filename'] not in ["", "-----"]:
            if current_dis['bib_src'] == 'txt':
                with open(current_dis['filename'], 'r') as f:
                    books_list = [
                        book.strip('\n\r').decode('utf-8')
                        for book in f.readlines() if len(book) > 10
                    ]
            else:
            ...
            G.node[c_sh]['color'] = year2color(
                current_dis['year']
            )  # ("%2x" % (int(current_dis['year']) - 1900))*3

            # Разбор библиографической записи
            # Самое длинное поле -- название?
            for book_string in books_list:
                book_lang = ''
                book_txt = book_string[book_string.find('.') +
                                       1:]  # Отбросить число в начале ссылки
                while book_txt[0] == ' ':
                    book_txt = book_txt[1:]  # Отбросить пробелы в начале
                ...
                for a in authors:
                    ...
                    if (
                            book_year != '' and int(book_year) > 1990
                    ) and a_sh != d_sh and a_sh != c_sh:  # Исключить цитирование себя или своего руководителя
                        if a_sh not in books_by_authors:
                            books_by_authors[a_sh] = []
                        found = 0
                        for bk in [bk for bk, yr in books_by_authors[a_sh]
                    ...
                    for gn in list(
                            G.nodes()
                    ):  # поиск наиболее похожего узла (gn -- кандидат на похожесть)
                        if gn != a_sh and Levenshtein.ratio(
                                a_sh[1:-2], gn[1:-2]
                        ) > 0.75 and a_sh[0] == gn[0] and a_sh[-2:] == gn[-2:]:
                            # нашли узел который уже подходит на роль автора
                            target_node = gn
                            # первая буква и инициалы совпадают, остальное совпадает ~до 2 букв
                            # занесём автора-кандидата a_sh в список похожих фамилий
                            if target_node not in equi: equi[target_node] = []
                            elif a not in equi[target_node]:
                                equi[target_node].append(a)
                            # словарь подстановки ключей <= для этого автора (его ключа) принята следующая подстановка
                            keys_equi[a_sh] = target_node
                            break
                            # Считаем что автор совпадает
                    ...
                    if target_node != c_sh and target_node != d_sh:  # Без самоцитирования и цитирования руководителя
                        ...
                        if ('type' not in G.node[target_node]) or (
                                'type' in G.node[target_node]
                                and G.node[target_node]['type'] != 'diss'):
                ...
                for pair in itertools.permutations(authors, 2):
                    key0 = name2key(pair[0])
                    key1 = name2key(pair[1])
                    # каждый узел может быть руководителем или аспирантом
        ...
        if 'type' in G.node[gn] and G.node[gn]['type'] == 'linkdiss':
            for a in diss_authors:
                if a in G.node[gn]['label']:
                    G.remove_node(gn)
                    break
                    # -> "новая" диссертация есть в первичном списке работ
            continue
        # Изменить размер узла соответственно количеству входящих стрелок
        in_edg = len([
            i for i in G.in_edges(gn)
            if 'type' in G.node[i[0]] and G.node[i[0]]['type'] != 'diss'
        ])
